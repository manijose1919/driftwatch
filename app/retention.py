...
from .models import DriftEvent, Snapshot, utcnow
...
def prune_old_data() -> dict:
    ...
    if events_deleted or snapshots_deleted:
        log.info(
            "retention prune: removed %d event(s), %d snapshot(s) older than %d day(s)",
            events_deleted, snapshots_deleted, settings.retention_days,
        )
    return {"events": events_deleted, "snapshots": snapshots_deleted}
